import { OpenAiRequest } from "#/lib/types/open-ai";
import {
  BOT_TRAINING_ORDER,
...
export const getBotParam = (
  bot: Bot | null,
  param: keyof Omit<OpenAiRequest, "stream" | "n">,
...
export const getManyTrainingContents = (
  ...
  try {
    ...
    return data.map((training) =>
      !!training ? training.attributes.content : null,
    );
  } catch (error) {
...
export const getCustomTriggerContents = (
  triggers?: BotTrigger[],
): (string | null)[] => {
  ...
  try {
    return triggers.map(
      (trigger) =>
        `If the user mentions ${trigger.phrase}, respond with: ${trigger.response}`,
    );
  } catch (error) {
...
export const getTrainingContent = (
  training?: CmsSingleRelation<BotTraining>,
): string | null => {
...
export const collateBotTraining = (bot: Bot | null): string => {
  ...
  const trainingArray = BOT_TRAINING_ORDER.map(
    (item) => trainingObject[item],
  ).flat();
