...
import { GptMessage, OpenAiChatModel, OpenAiModel } from "#/lib/types/open-ai";

export const calculateTokens = (prompt: string) => {
  // TODO - Need a way to calculate tokens
  return undefined;
};
...
export const createSuggestionsPrompt = (context: StatusChatMessage[]) => {
  const recentMessages =
    context.length > DEFAULT_SUGGESTIONS_MEMORY
      ...
      : context;
  const messages = recentMessages.map(({ role, content }) => ({
    role,
    content,
  }));
  const messageQuery = [
    ...messages,
    { role: "user", content: SUGGESTIONS_REQUEST } as GptMessage,
  ];
...
export const createSubmissionsPrompt = (context: StatusChatMessage[]) => {
  const recentMessages =
    context.length > SUBMISSIONS_PROMPT_SIZE
      ? context.slice(SUBMISSIONS_PROMPT_SIZE)
      : context;
  const messages = recentMessages.map(({ role, content }) => ({
    role,
    content,
  ...
  const messageQuery = [
    ...messages,
    { role: "user", content: SUBMISSIONS_REQUEST } as GptMessage,
  ];

  return messageQuery;
};

export const getStartingChatLog = (
  bot?: Bot | null,
): StatusChatMessage[] | [] => {
  if (!bot) return [];
...
export const filterConversationsData = (
  ...
  const filteredFolders = folders.filter(
    (folder) =>
      filterConversations(folder.conversations, searchQuery).length > 0,
  );
  const filteredRootConversations = filterConversations(
    rootConversations,
    searchQuery,
  );
  return {
    folders: filteredFolders,
    rootConversations: filteredRootConversations,
  };
};
...
export const compileChatMessages = (
  ...
  const recentMessages = overLimit
    ? messages.slice(1).slice(-sliceLength)
    : messages;
  const messagesWithTimestamps: StatusChatMessage[] = [
    ...(recentMessages || []),
    latestMessage,
  ];
  ...
  const allChatMessages: StatusChatMessage[] = [
    ...(messages || []),
    latestMessage,
  ];
